...
Chunk ChunkManager::get_chunk_from_position(float x, float y) const {
	...
	auto pos = RenderManager::get().map_pixel_to_coords(sf::Vector2i(x, y));
...
void ChunkManager::draw_debug_chunks() {
	...
	for (auto x = beg.x;
		...
		auto rect_size = sf::Vector2f(m_chunk_size, m_chunk_size);
		...
		for (auto y = beg.y;
			...
			sf::RectangleShape rectangle;
			rectangle.setPosition(sf::Vector2f(x + 1.f, y + 1.f));
			rectangle.setSize(rect_size - sf::Vector2f(2.f, 2.f));
			auto color = sf::Color(50 * count_colliding, 0, 255, 200);
